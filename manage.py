    ...
    async def ban(self, ctx):
        ...
        for target in ctx.message.mentions:
            try:
                ...
                await ctx.send(f"Failed to ban user <@{target.id}>")
            else:
                target_users.add(str(target.id))
        if not target_users:
            return
    ...
    async def kick_together(self, ctx):
        ...
        for target in target_users:
            try:
                ...
                await ctx.send(f"Failed to kick user <@{target}>")
            else:
    ...
    async def ban_together(self, ctx):
        ...
        for target in target_users:
            try:
                ...
                print(traceback2.format_exc())
                await ctx.send(f"Failed to ban user <@{target}>")
            else:
                target_checked.add(str(target))
        if not target_checked:
            return
